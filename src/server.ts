...
const runningServers = new Set<http.Server | https.Server>();
let totalRequests = 0;
...
const publishDiagnosticEvent = (event: { requestId: string; timestamp: string; method: string; path: string; statusCode: number }) => {
...
if (!isCodespaces && (!fs.existsSync(keyPath) || !fs.existsSync(certificatePath))) {
  if (isProduction) throw new Error('Production TLS certificate files are required.');
...
const listenOnPort = (server: http.Server | https.Server, port: number, label: string): Promise<void> =>
  new Promise((resolve, reject) => {
    const onError = (error: NodeJS.ErrnoException) => reject(new Error(`${label} port ${port} is unavailable: ${error.message}`));
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (diagnosticEvents.length > 25) diagnosticEvents.shift();
    publishDiagnosticEvent(diagnosticEvent);
  });
  next();
});
app.use(express.json({ limit: maxRequestBytes }) as express.RequestHandler);
app.use(helmet({
...
}) as express.RequestHandler, (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ error: 'Invalid catalog query' });
...
app.get('/api/products/:id', (req, res) => {
  if (!/^[a-z0-9-]{1,64}$/.test(req.params.id)) return res.status(400).json({ error: 'Invalid product id' });
...
app.get('/api/example',
  [query('name').optional().isString().trim().isLength({ max: 100 }).withMessage('Name must be at most 100 characters')],
  async (req, res) => {
    const errors = validationResult(req);
...
].filter((user): user is { username: string; role: string; passwordHash: string } => Boolean(user.passwordHash));
...
app.post('/api/data-pipe', authLimiter, authenticate, (req, res) => {
  ...
  writeStream.on('error', (error) => {
    ...
    if (!res.writableEnded) res.end('Error processing stream');
  });
  req.on('error', (error) => {
...
app.use((error: any, _req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (error?.code === 'EBADCSRFTOKEN') return res.status(403).json({ error: 'Invalid CSRF token' });
...
const shutdown = async (signal: string) => {
  ...
  for (const subscriber of diagnosticSubscribers) subscriber.end();
  diagnosticSubscribers.clear();
  await Promise.all([...runningServers].map((server) => new Promise<void>((resolve) => {
